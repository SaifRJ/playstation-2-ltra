...
function attachSelector(targetMesh) {
    ...
    const handle = {
        ...
        target: null,
        orbitRadius,
        center: new THREE.Vector3(),   // the moving point orbs orbit around
        ...
        update(t) {
            ...
            orbs.forEach(o => {
                ...
                if (o.history.length > HISTORY) o.history.length = HISTORY;

                const pos = o.trail.geometry.attributes.position.array;
                const col = o.trail.geometry.attributes.color.array;
                for (let i = 0; i < HISTORY; i++) {
                    const p = o.history[i] || o.history[o.history.length - 1] || [0,0,0];
                    pos[i*3] = p[0]; pos[i*3+1] = p[1]; pos[i*3+2] = p[2];
                    const fade = 1 - (i / HISTORY);
                    col[i*3]   = o.color.r * fade;
                    col[i*3+1] = o.color.g * fade;
                    col[i*3+2] = o.color.b * fade;
                }
...
function setSelectorTarget(mesh) {
    ...
    mid.y += 0.6;  // bulge for the curve — tune to taste, 0 = straight line
    ...
    migrationTween = gsap.to(progress, {
        ...
        ease: "power2.inOut",
        onUpdate: () => {
            curve.getPointAt(progress.u, activeSelector.center);
        },
        onComplete: () => { migrationTween = null; }
    });
